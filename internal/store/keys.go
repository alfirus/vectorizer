...
func (s *Store) QueryConclusions(workspaceID, query string, n int) ([]map[string]interface{}, error) {
	// semantic search over conclusions collection
	collName := fmt.Sprintf("ws_%s_conclusions", workspaceID)
	...
	for _, r := range results { out = append(out, map[string]interface{}{"id": r.ID, "document": r.Document, "metadata": r.Metadata, "distance": r.Distance}) }
	return out, nil
}
