...
export function registerPeerTools(server: McpServer, getClient: () => Client) {
  server.tool("vectorizer_create_peer", "Create peer in workspace ()", { workspace_id: z.string(), peer_id: z.string()}, async (a) => {
  ...
  server.tool("vectorizer_chat", "Dialectic chat (peer-aware RAG + peer_cards, peer chat)", { workspace_id: z.string(), query: z.string(), observer: z.string().optional(), observed: z.string().optional(), session_id: z.string().optional()}, async (a) => {
